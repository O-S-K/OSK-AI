...
public class FSMDebug : EditorWindow
{
                        ...
                        if (states != null)
                        {
                            using (new EditorGUILayout.HorizontalScope())
                            {
                                foreach (var s in states)
                                {
                                    ...
                                    if (GUILayout.Button($" {ShortStateName(s)} ", st, GUILayout.Height(18)))
                                    {
                                        selectedFSMOwner = d.owner;
                                        selectedStateObj = s;
                                    }
        ...
        using (new EditorGUILayout.VerticalScope("box", GUILayout.ExpandWidth(true)))
        {
            ...
            else
            {
                foreach (var t in allTrans)
                {
                    ...
                    bool cond = false;
                    try { cond = t.Condition?.Invoke() ?? false; } catch { cond = false; }
                    ...
                    // color bracket part
                    string colorTag = cond ? "<color=#FFD966>" : "<color=#9B9B9B>";
                    string bracket = $"{colorTag}[{desc} {(cond ? "✅" : "✖")}]</color>";

                    string line = $"- [{t.Priority}] {ShortStateName(left)} → {ShortStateName(right)}   {bracket}{(string.IsNullOrEmpty(evalInfo) ? "" : $" <color=#888888>{evalInfo.Trim()}</color>")}{(string.IsNullOrEmpty(trigInfo) ? "" : $" <color=#888888>{trigInfo.Trim()}</color>")}";
            ...
            else
            {
                foreach (var at in anyTransitions)
                {
                    ...
                    bool cond = false;
                    try { cond = at.Condition?.Invoke() ?? false; } catch { cond = false; }
                    ...
                    string line = $"- [{at.Priority}] → {ShortStateName(to)}   {bracket}{(string.IsNullOrEmpty(evalInfo) ? "" : $" <color=#888888>{evalInfo.Trim()}</color>")}{(string.IsNullOrEmpty(trigInfo) ? "" : $" <color=#888888>{trigInfo.Trim()}</color>")}";
    ...
    private void DrawStateFields(IState state)
    {
        ...
        {
            object v = null;
            try { v = f.GetValue(state); } catch { v = "(err)"; }
            ...
            try
            {
                ...
                foreach (var p in props)
                {
                    if (p.PropertyType == typeof(FinalStateMachine) && p.GetIndexParameters().Length == 0)
                    {
                        ...
                        try { fsm = p.GetValue(mb) as FinalStateMachine; } catch { fsm = null; }
                        if (fsm != null)
                        {
    ...
    private class DetectedFSM
    {
        ...
        {
            try { return fsm.GetCurrentState(); } catch { return null; }
        }

        public List<IState> GetStates()
        {
            try { return fsm.GetStates(); } catch { return null; }
        }
        ...
        {
            try { return fsm.GetAnyTransitions(); } catch { return null; }
        }

        public int GetTransitionsCount() { var all = GetAllTransitions(); return all?.Count ?? 0; }
        public int GetAnyTransitionsCount() { var a = GetAnyTransitions(); return a?.Count ?? 0; }
    }
}
#endif
